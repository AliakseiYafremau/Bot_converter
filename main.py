        ...
        def convert(message: telebot.types.Message):
            try:
                ...
                for currency in currencies:
                    index_currency = message.text.find(currency)
                    if index_currency != -1:
                ...
                if base == quote:
                    raise ConvertException('Нельзя перевести деньги в туже валюту')

                amount = message.text.removeprefix(base).strip()
                ...
                bot.send_message(message.chat.id, f'{amount} {currencies[base]} будет равно {result} {quote}')
